...
max <- rep.int(sar[nrow(sar), "Time Since Start"], nrow(sar))
max1 <- unlist(max)
sar$Total=max1
...
write_tsv(cpu, paste("cpu.tsv",sep = ""))
write_tsv(mem, paste("mem.tsv",sep = ""))
write_tsv(net, paste("net.tsv",sep = ""))
write_tsv(disk, paste("disk.tsv",sep = ""))
