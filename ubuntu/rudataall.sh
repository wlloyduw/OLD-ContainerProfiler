...
then
  ...
  runDataPids=( $(pgrep rudataall.sh) )

  #store the children in a associative array
  declare -A runDataChild=()

  #find children of rudataall.sh pids
  for runDataPid in ${runDataPids[@]}
  do 
    newChildren=(  $(pgrep -P $runDataPid) )
    for child in ${newChildren[@]}
    do
      runDataChild[$child]=1
    done
  done

  #find the non rudataAll.sh processes
  nonProfilerPids=( $(pgrep "rudataall.sh" -v) )

  #check them against the children before processing
  for pid in ${nonProfilerPids[@]}
  do
    if [ -z ${runDataChild[$pid]} ]; then
	...
	if (( ${#STAT[@]} )); then
	  ...
	  VCSWITCH=$(awk '$1 == "voluntary_ctxt_switches:" { print $2}' /proc/1/status)
	  NVCSSWITCH=$(awk '$1 == "nonvoluntary_ctxt_switches:" { print $2}' /proc/1/status)

	  a=$(grep ctxt /proc/$pid/status)
	  b=(grep ctxt /proc/$pid/status)
